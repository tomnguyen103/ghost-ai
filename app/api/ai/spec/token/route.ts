...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error("[ai/spec/token] createPublicToken failed:", msg);
      return NextResponse.json({ error: "Could not create public token" }, { status: 503 });
    }

    return NextResponse.json({ token: publicToken });
  } catch (err) {
    console.error("[ai/spec/token] unhandled error:", err);
